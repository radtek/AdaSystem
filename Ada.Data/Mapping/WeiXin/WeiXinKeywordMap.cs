...
{
   public class WeiXinKeywordMap : EntityTypeConfiguration<WeiXinKeyword>
    {
        public WeiXinKeywordMap()
        {
            //配置主键
            HasKey(s => s.Id);
            //配置字段
            ...
            //配置表
            ToTable("WeiXinKeyword");
            //配置关系【一对多的配置，外键是UserId】 Withmany方法允许多个。HasForeignKey方法表示哪个属性是User表的外键，WillCascadeOnDelete方法用来配置是否级联删除
            HasRequired(s => s.WeiXinAccount).WithMany(s => s.WeiXinKeywords).HasForeignKey(s => s.WeiXinAccountId).WillCascadeOnDelete(false);
        }
    }
}
